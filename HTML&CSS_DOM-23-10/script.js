...
for(var row=0;row<10;row++){
    ...
    for(var col=0;col<3;col++){ 
        ...
        var id = "row"+ row.toString() + "col"+col
...
function paginationflow(pgno){
    document.getElementById(pgno).setAttribute('class', 'active')
    var enditem = pgno*10
    var startitem = enditem-10
    for(var row=0;row<10;row++){
        for(var col=0;col<3;col++){ 
            var id = "row"+ row.toString() + "col"+col
            document.getElementById(id).innerHTML= (data[startitem])[Object.keys(data[startitem])[col]]
        }
        startitem++;
    }
}
...
for(var page=1;page<=10;page++){
    ...
    anchor.setAttribute('onclick', 'paginationflow('+page+')')
